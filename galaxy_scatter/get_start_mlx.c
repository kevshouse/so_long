...
/*   Updated: 2025/03/04 19:31:52 by keanders         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void generate_point_clusters(t_data *data, int num_clusters, int points_per_cluster)
{
...
/*int	main(void)
{
	...
	v2 = (t_vertx){500, 200};
	mlx = mlx_init();
	mlx_win = mlx_new_window(mlx, 1024, 768,
			"Hi 42, and thanks for all the fish!");
	...
	while (i < 10000)
	{
		random_x = rand() % 1024;
		random_y = rand() % 768;
		ft_put_pixel(&img, random_x, random_y, random_color());
		mlx_put_image_to_window(mlx, mlx_win, img.img, 0, 0);
		i++;
	}
	
	//ft_draw_line(&img, v1, v2, 0x00FF0000);
	// Draw a line between v1 and v2 with a random color
	srand(time(NULL));
	ft_draw_line(&img, v1, v2, random_color()); // Random color
	mlx_put_image_to_window(mlx, mlx_win, img.img, 0, 0);
...
int	main(void)
{
	...
	generate_point_clusters(&img, 5, 2000); // 5 clusters, 2000 points per cluster
